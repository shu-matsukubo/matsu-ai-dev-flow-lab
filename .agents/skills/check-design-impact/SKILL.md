...
設計影響確認の開始前にMainが最新Issueのステータスを確認する。`AI：作業可能`だけで現在のチャット指示がある場合に限り開始し、未付与・人間承認待ち・複数競合・永続情報との不整合では変更せず停止する。設計PR作成または設計変更不要の根拠提示後はMainが`人間：基本設計承認待ち`へ切り替え、同じ指示でTask Planningへ進まない。
...
設計文書の変更を機械的に要求しない。影響がなければ、変更不要の理由と確認した正本をRequirement Issueへコメントし、`人間：基本設計承認待ち`へ引き渡して終了する。同じ作業指示でタスク分解へ進まない。人間が判断を承認し、`AI：作業可能`へ切り替えて新しい指示を行った後、最新の正本から設計影響を再確認し、追加の設計変更が不要な場合にだけ`$plan-tasks`へ進む。
