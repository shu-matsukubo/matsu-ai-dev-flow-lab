...
Mainは各作業指示の開始時に最新Issueの4種類のステータスラベルを確認し、`AI：作業可能`だけと現在のチャット指示がある場合に限りWorkerへ実装を委譲する。未付与・人間承認待ち・複数競合・旧3ラベルの残存・永続情報との不整合では移行完了まで変更せず停止する。Mainは自己付与せず、Worker・Reviewerはラベルを変更しない。Task完了後のIssue統合PRを含む完成PRは`人間：PR確認待ち`へ引き渡す。一部TaskやTask PRの公開だけでは最終確認待ちへ切り替えない。 Mainは完成成果物の引き渡し時に非ステータスラベルを保持し、以前のステータスを残さず目的の`人間：PR確認待ち`へ更新し、更新後Issueを再取得して目的のステータス1種類だけであることを確認する。更新または確認に失敗した場合、または同一指示内では次工程へ進まず停止する。Task PRだけではこの状態変更を行わない。
