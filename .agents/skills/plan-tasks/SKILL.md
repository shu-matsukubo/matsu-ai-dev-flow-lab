...
Task Planning開始前にMainが最新Issueの4種類のステータスを確認し、`AI：作業可能`だけと現在のチャット指示を要求する。未付与・人間承認待ち・複数競合・旧3ラベルの残存・永続情報との不整合では移行完了まで計画やfileを作成せず安全停止する。計画提示後はMainが`人間：タスク承認待ち`へ切り替え、同じ指示でbranch、PR、Task fileを作成しない。

## 前提と参照する正本

`$check-design-impact` が作成した設計判断記録と設計PRの人間mergeを確認してから使用する。Requirement Issue、merge済み要求分析書、現在の`develop`、`docs/`、関連実装を読む。Issueは要求原文と事前の補足、要求分析書は目的、要件、受入条件、制約、対象外、人間判断の正本として扱い、過去の未着手計画を正本として復元しない。要求分析書が存在しない、またはRequirement Analysis PRが未mergeなら使用しない。

設計PRがmergeされる前のチャットでは使用しない。設計PRがmergeされた場合も、そのチャットの未永続な計画を引き継がず、別チャットでRequirement Issue、merge済み要求分析書、最新の`develop`、最新の`docs/`から設計影響確認をやり直し、merge済み設計PRを確認できた場合にだけ使用する。
