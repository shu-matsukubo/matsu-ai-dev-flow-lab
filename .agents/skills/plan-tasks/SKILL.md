...
Task Planning開始前にMainが最新Issueのステータスを確認し、`AI：作業可能`だけと現在のチャット指示を要求する。未付与・人間承認待ち・複数競合・永続情報との不整合では計画やfileを作成せず停止する。計画提示後はMainが`人間：タスク承認待ち`へ切り替え、同じ指示でbranch、PR、Task fileを作成しない。
