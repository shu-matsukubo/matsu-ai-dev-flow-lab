...
公開作業の開始前にMainが最新Issueのステータスと永続情報を確認し、`AI：作業可能`だけで現在のチャット指示がある場合に限り進める。未付与・人間承認待ち・複数競合・不整合ではremoteやfileを変更せず停止する。Task PRの公開はIssue統合前の中間操作であり、公開直後に`人間：最終成果物承認待ち`へ遷移しない。全Task、最新`develop`同期、Issue統合・回帰検証、全受入条件確認後にIssue統合PRが人間確認可能となった場合だけ、Mainが最終承認待ちへ引き渡す。Worker・Reviewerはステータスを変更しない。
