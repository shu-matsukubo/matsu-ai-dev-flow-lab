...
レビュー開始前にMainが最新Issueの4種類の有効ステータスを確認し、`AI：作業可能`だけと現在のチャット指示、必要な永続情報が揃う場合に限りレビューする。旧3ラベル残存・未付与・人間承認待ち・複数競合・不整合ではレビュー成果物やコメント、ラベル変更を行わず移行完了まで安全停止する。Reviewerはステータスを変更せず、工程完了時の引き渡しはMainだけが行う。
