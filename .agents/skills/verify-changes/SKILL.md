...
検証開始前にMainが最新Issueの4種類の有効ステータスを確認し、`AI：作業可能`だけと現在のチャット指示、必要な永続情報が揃う場合に限り検証する。旧3ラベル残存・未付与・人間承認待ち・複数競合・不整合では成果物やラベルを変更せず移行完了まで安全停止する。検証完了後の人間承認待ちへの切り替えはMainだけが行い、Worker・Reviewerはステータスを変更しない。
