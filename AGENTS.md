...
必要な手順は、その時点で利用可能なWorkflow、Skill、Referenceを確認して選択する。

Skillは自己完結した単一能力として設計し、Skill間の直接依存を避ける。能力の構成、順序、停止・再開、状態遷移はWorkflowへ、複数能力で共有する判断知識はReferenceへ分離する。Skillの新規設計、変更、設計レビューでは、その時点で利用可能な設計ガイドを能力契約から選択して確認する。

高難度、高リスク、または判断困難な論点では、現在の承認scope内で利用可能な独立した専門レビュー能力を必要に応じて選択できる。依頼元は利用理由、対象、制約、結果、未確認事項、残るリスクを既存の正本へ必要十分に記録し、助言を現在の正本と照合して対応を決める。専門レビュー担当へ作業主体、承認、提出、外部変更、または最終判断を移さず、利用不能や入力不足を成功扱いしない。個別能力の名前や存在を固定的に仮定せず、`name`、`description`、適用条件、入力、出力、責務外を照合する。能力が存在しない、入力契約を満たせない、または安全に選べない場合は、無関係な文書を変更して補わず、未実施または不足として返す。
