...
ステータスラベルは`AI：作業可能`または`人間：PR確認待ち`の2種類だけで、次に行動する作業主体を示す。工程完了や人間承認の証拠を置き換えない。`AI：作業可能`だけでなく現在のチャット指示も揃う場合にだけ作業を開始し、要求、設計、Task、Pull Request、diff、検証結果を直接確認して現在工程を復元する。旧状態、未付与、複数状態、永続成果物との矛盾があれば安全停止する。
