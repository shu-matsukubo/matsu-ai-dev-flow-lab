...
FrontからAPIへソースコードを共有しない。共有が必要に見える場合も、shared packageを先に導入せずDesign Impact Checkで契約とownershipを再検討する。
...
service境界、dependency方向、API契約方式、認証・認可・session、persistence・data ownership、security boundary、testing strategy、CI gate、AI開発フロー、Agent / Skill責務を変える要求は、実装前に [AI開発フロー](../ai-development/overview.md) のDesign Impact Checkを行う。現在存在しない仕組みを将来の便宜だけで追加しない。
